...
int Tetraedro::calcLocalMatrix(std::map<unsigned long,Node> *list_nodes)
{
	...
	local[0] = (b1*b1+c1*c1+d1*d1);
	local[1] = (b1*b2+c1*c2+d1*d2);
	local[2] = (b1*b3+c1*c3+d1*d3);
	local[3] = (b1*b4+c1*c4+d1*d4);
	local[4] = local[1];
	local[5] = (b2*b2+c2*c2+d2*d2);
	local[6] = (b2*b3+c2*c3+d2*d3);
	local[7] = (b2*b4+c2*c4+d2*d4);
	local[8] = local[2];
	local[9] = local[6];
	local[10] = (b3*b3+c3*c3+d3*d3);
	local[11] = (b3*b4+c3*c4+d3*d4);
	local[12] = local[3];
	local[13] = local[7];
	local[14] = local[11];
	local[15] = (b4*b4+c4*c4+d4*d4);
...
double Tetraedro::getFatorCondut() {
	return 1/(36*volume*condutividade);
}

double Tetraedro::getDerivCondut() {
